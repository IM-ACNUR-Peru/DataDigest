...
```{r snm_ridl, dpi = 200,  echo=FALSE, warning=FALSE, fig.height=3.2}
...
```
Entre el 1/Ene 2023 y el `r day(max(snm_data$Update))`/`r snm_month` de 2025, el **saldo total de flujos regulares para estas 4 nacionalidades ha sido `r saldo_total`**.
...
```{r snm_frontera, dpi = 200,  echo=FALSE, warning=FALSE, fig.height=6.5}
...
::: column
```{r snm_puntos, dpi = 200,  echo=FALSE, warning=FALSE, fig.height=6.5}
...
stock_data <- queryTable("c9rxrtmlyt3ar193") |> 
  select(Unidad.Name, Tipo.de.kit.CRI, Cantidad.en.stock, Fecha.de.actualizacion) |> 
  rename(region = Unidad.Name) |> 
  rename(Detalle = Tipo.de.kit.CRI) |> 
