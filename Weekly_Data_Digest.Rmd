...
# Set this if headless chrome works to produce a pdf


#Sys.setenv(PAGEDOWN_CHROME = "C:/Program Files/Google/Mozilla Firefox/firefox.exe")
...
pbi <- function(file, location, pob) {
  ...
  r <- POST(url, body = query, add_headers("X-PowerBI-ResourceKey" = "84e535e3-6ec4-4f8f-9fbf-778745af0146"))
...
Dashboard DTM de conteos en frontera: [ ](https://app.powerbi.com/view?r=eyJrIjoiODRlNTM1ZTMtNmVjNC00ZjhmLTlmYmYtNzc4NzQ1YWYwMTQ2IiwidCI6IjE1ODgyNjJkLTIzZmItNDNiNC1iZDZlLWJjZTQ5YzhlNjE4NiIsImMiOjh9)
...
## Flujos regulares de 4 nacionalidades
```{r snm_ridl_setup, echo=FALSE, warning=FALSE, message = FALSE, include=FALSE}


file_url <- "https://ridl.unhcr.org/dataset/60d1ea06-4bd0-4cb8-bcc4-d563e63f7ab1/resource/f4d35ee7-8f1a-421b-b961-a6e9ad4029b8/download/snm_flows.xlsx"

df <- riddle::resource_fetch(url = file_url,   path =  here::here("SNM_Flows.xlsx"))

snm_data <- read_excel(df) |> 
  dplyr::filter(Update == max(Update)) |> 
  dplyr::filter(grepl("2024|2023", Mes)) |> 
  filter(Nacionalidad == "Ecuatoriana" | Nacionalidad == "Boliviana" | Nacionalidad == "Venezolana" | Nacionalidad == "Colombiana")

file.remove("SNM_Flows.xlsx")
...
La Superintendencia Nacional de Migraciones reporta los siguientes flujos de ingreso y salidas regulares de personas bolivianas, colombias, ecuatorianas y venezolanas durante los últimos 12 meses:
```{r snm_ridl, dpi = 200,  echo=FALSE, warning=FALSE, fig.height=3.5}
...
tot_snm <- snm_data |> 
  ...
  dplyr::mutate(Tot_Days_Mes = if_else(year(Mes) != year(Sys.Date()), Tot_Days_Mes, if_else(month(Update) == month(Mes), as.numeric(day(Update)), Tot_Days_Mes))) |> 
...
```
Entre el 1/Ene 2023 y el `r day(max(snm_data$Update))`/`r snm_month` de 2024, el **saldo total de flujos regulares para estas 4 nacionalidades ha sido `r saldo_total`**.
...
```{r snm_frontera, dpi = 200,  echo=FALSE, warning=FALSE, fig.height=9}
...
::: column
```{r snm_puntos, dpi = 200,  echo=FALSE, warning=FALSE, fig.height=9}
...
```{r snm_ridl_ecu, echo=FALSE, warning=FALSE}
# Entries and exits total since Jan 2023 by month
...
tot_snm_ecu <- snm_data |> 
  ...
  mutate(Update = ymd(Update)) |> 
  mutate(Tot_Days_Mes = days_in_month(Mes)) |> 
  mutate(Tot_Days_Mes = if_else(month(Update) == month(Mes) & year(Update) == year (Mes), as.numeric(day(Update)), Tot_Days_Mes)) |> 
  filter(Tot_Days_Mes > 2) |> #With just one day of data results can be inaccurate
  mutate(Promedio_Diario = round(Total / Tot_Days_Mes, 0)) 
...
#    annotate("rect", xmin = max(last_data_dtm$date), xmax = as.Date(Inf), ymin = -Inf, ymax = Inf, fill = "white", alpha = 0.8) +
  ...
  labs(
    ...
    subtitle = "Promedio de flujos diarios por mes",
    y = "Personas",
    caption = "Fuente: SNM"
  ) +
#   scale_x_continuous(
...
::: column
```{r snm_ridl_col, echo=FALSE, warning=FALSE}
# Entries and exits total since Jan 2023 by month
...
```{r snm_ridl_bol, echo=FALSE, warning=FALSE}
# Entries and exits total since Jan 2023 by month
...
```{r snm_ridl_ven, echo=FALSE, warning=FALSE}
# Entries and exits total since Jan 2023 by month
...
kits_cris_list_trend_pred <- rudah_all |>
  select(region, today, qt_kit_higiene_familiar, qt_kit_dignidad, starts_with("qt_cri")) |>
...
```{r, echo=FALSE, warning = FALSE, message = FALSE,  fig.width=7,fig.height=4.5}
...
kits_cris_list_trend_detail_socios <- rudah_all |>
  ...
  select(region, today, organizacion, qt_kit_higiene_familiar, qt_kit_dignidad, starts_with("qt_cri")) |>
  filter(today > referencemin) |> 
  mutate_at(vars(starts_with("qt_kit")), ~ replace(., is.na(.), 0)) |>
  mutate_at(vars(starts_with("qt_kit")), as.numeric) |>
  mutate_at(vars(starts_with("qt_cri")), ~ replace(., is.na(.), 0)) |>
...
stock_data <- queryTable("c9rxrtmlyt3ar193") |> 
  ...
  group_by(region, Detalle, Update) |> 
  summarise_at(vars("Stock"), sum, na.rm = TRUE)
...
brecha <- stock_data |> 
  ...
  mutate_at(vars(last_col()), ~ replace(., is.na(.), 0)) |>
...
#   select(region, today, organizacion, qt_kit_higiene_familiar, qt_kit_dignidad, starts_with("qt_cri")) |>
#   mutate_at(vars(starts_with("qt_")), ~ replace(., is.na(.), 0)) |>
#   mutate_at(vars(starts_with("qt_")), as.numeric) |>
#    group_by(region, today, organizacion) |> 
#   summarise_at(vars(starts_with("qt_")), sum, na.rm = TRUE) 
# 
# 
