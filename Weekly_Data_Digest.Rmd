...
months <- ds$ValueDicts$D0[[1]]

eys <- 
  ds$PH[[1]]$DM0[[1]]$X |> list_rbind() |> as_tibble() |> select(-S) |> 
  mutate(direction = rep_len(c("entradas", "salidas"), n()))

when <- 
  ds$PH[[1]]$DM0[[1]]$C |> 
  map(\(x) tibble(month = if_else(length(x) == 2, x[1], NA), 
                  day = if_else(length(x) == 2, x[2], x[1]))) |> 
  list_rbind() |> 
  fill(month) |> 
  mutate(month = months[1+month])

nat_data_dtm <- bind_cols(when |> mutate(n = 2) |> uncount(n), eys) |> 
  pivot_wider(names_from = direction, values_from = M0) |> 
...
tacna_data_dtm <- bind_cols(when |> mutate(n = 2) |> uncount(n), eys) |> 
  pivot_wider(names_from = direction, values_from = M0) |> 
  mutate(level = "Tacna")
...
tumbes_data_dtm <- bind_cols(when |> mutate(n = 2) |> uncount(n), eys) |> 
  pivot_wider(names_from = direction, values_from = M0) |> 
  mutate(level = "Tumbes")
...
puno_data_dtm <- bind_cols(when |> mutate(n = 2) |> uncount(n), eys) |> 
  pivot_wider(names_from = direction, values_from = M0) |> 
  mutate(level = "Puno")
...
ggplot(all_data_dtm, aes(
 ...
 # scale_x_continuous(breaks = pretty_breaks()) +
  scale_x_date(date_labels="%b %Y", date_breaks  ="2 months", limits = c(min(all_data_dtm$date), max(all_data_dtm$date)+30)) +
 #  scale_y_continuous(
...
#    limits = c(0, ymax_kits_cris_list_trend)
  ...
  facet_wrap(~ level, nrow = 4, scales='free') +
  theme(
...
ggplot(kits_cris_list_trend_all, aes(
 ...
 # scale_x_continuous(breaks = pretty_breaks()) +
  scale_x_date(date_labels="%b %Y", date_breaks  ="1 month", limits = c(min(kits_cris_list_trend_all$today), max(kits_cris_list_trend_all$today)+30)) +
  scale_y_continuous(
    expand = expansion(c(0, 0.1)),
    breaks = pretty_breaks(),
    labels = label_number(scale_cut = cut_short_scale()),
...
Durante los últimos 30 días, la operación brindó `r total_ori_gen_month` orientaciones generales y `r total_ori_legal_month` orientaciones legales a un total de `r personas_ori_tot_month` personas`r tot_ori_statement`. Durante los últimos días, se brindaron `r total_ori_gen` orientaciones generales y `r total_ori_legal` orientaciones legales.
...
ggplot(rudah_ori_plot_all, aes(
 ...
 # scale_x_continuous(breaks = pretty_breaks()) +
  scale_x_date(date_labels="%b %Y", date_breaks  ="1 month", limits = c(min(rudah_ori_plot_all$today), max(rudah_ori_plot_all$today)+30)) +
  scale_y_continuous(
    expand = expansion(c(0, 0.1)),
    breaks = pretty_breaks(),
    labels = label_number(scale_cut = cut_short_scale()),
