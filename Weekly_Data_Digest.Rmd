...
main_2023 <- x$main |> 
  select("today":"perfil", "tipo_orientacion":"paisprevio", "paisdestino", "cri_dis_balde":"cri_dis_teclado", "kit_dis_kit_abrigo_bebe":"kit_dis_other", "member_count")
member2_2023 <- x$member


# RUDAH 2024
x <- kobo_submissions("aMRCvAGEpGSMsvS4A2VMzC")
main_2024 <- x$main |> 
  rename(
...
member2_2024 <- x$member

main <- bind_rows(main_2023, main_2024)
member2 <- bind_rows(member2_2023, member2_2024)
...
```{r dpi = 200, echo=FALSE, warning=FALSE, message=FALSE, fig.align = 'center', results='hide', fig.width=7}

url <- "https://wabi-north-europe-f-primary-api.analysis.windows.net/public/reports/querydata?synchronous=true"


# National data
query <- read_file("dtm_national_new_2.json")
...
# Tacna data
query <- read_file("dtm_tacna_new_2.json")
r <- POST(url, body = query, add_headers("X-PowerBI-ResourceKey" = "f4238ef3-6559-492b-95ac-f285a3a62588"))
...
# Tumbes data
query <- read_file("dtm_tumbes_new_2.json")
...
# Puno data
query <- read_file("dtm_puno_new_2.json")
...
ggplot(all_data_dtm, aes(
 ...
 #   title = "Flujos en frontera",
  ...
  scale_x_date(date_labels="%b %Y", date_breaks  ="2 months",
               limits = c(min(all_data_dtm$date), max(all_data_dtm$date)+80)
               ) +
   scale_y_continuous(
...
Dashboard DTM de conteos en frontera: [ ](https://app.powerbi.com/view?r=eyJrIjoiMGQxYWMxYzItYzlmMi00MjAzLWE0YzgtNGNiNjczYjMzOTkwIiwidCI6IjE1ODgyNjJkLTIzZmItNDNiNC1iZDZlLWJjZTQ5YzhlNjE4NiIsImMiOjh9)
...
rudah_kits_cris_plot <- rudah |>
  select(region, "qt_kit01":"qt_kit_other", "qt_cri01":"qt_cri_other") |>  
  ...
  mutate_at(vars(starts_with("qt_cri")), as.numeric) |>
   mutate(Kits = rowSums(across("qt_kit01":"qt_kit_other"))) |>
  mutate(CRIs = rowSums(across("qt_cri01":"qt_cri_other"))) |>
...
total_kits_cris_line <- rudah |> 
 select(today, "qt_kit01":"qt_kit_other", "qt_cri01":"qt_cri_other") |>  
  mutate_at(vars(starts_with("qt_kit")), ~ replace(., is.na(.), 0)) |>
  mutate_at(vars(starts_with("qt_kit")), as.numeric) |>
  mutate_at(vars(starts_with("qt_cri")), ~ replace(., is.na(.), 0)) |>
  mutate_at(vars(starts_with("qt_cri")), as.numeric) |>
...
xls <- kobo_form("a9Erto2aYbgJamz8vdrtJt") |> 
  select(name, label) |> 
...
kits_cris_list_trend_all <- rudah_all |>
  ...
  mutate_at(vars(starts_with("qt_cri")), ~ replace(., is.na(.), 0)) |>
  mutate_at(vars(starts_with("qt_cri")), as.numeric) |>
  pivot_longer(cols = c("qt_kit01":"qt_cri_other"), names_to="Tipo", values_to="Total") |>
...
kits_cris_list_trend <- rudah_all |>
  select(region, today, "qt_kit01":"qt_kit_other", "qt_cri01":"qt_cri_other") |>
  mutate_at(vars(starts_with("qt_kit")), ~ replace(., is.na(.), 0)) |>
  mutate_at(vars(starts_with("qt_kit")), as.numeric) |>
  mutate_at(vars(starts_with("qt_cri")), ~ replace(., is.na(.), 0)) |>
  mutate_at(vars(starts_with("qt_cri")), as.numeric) |>
...
kits_cris_list_trend_detail <- rudah_all |>
  ...
  summarise_at(vars("qt_kit01":"qt_cri_other"), sum, na.rm = TRUE) 
...
kits_cris_list_trend_detail <- kits_cris_list_trend_detail |>
  ...
  mutate_at(vars("qt_kit01":"qt_cri_other"), list(~zoo::rollmean(., 7, fill = NA, align = "center"))) |> 
  na.omit()|> 
  filter(today > referencemin) |> 
  na.omit() |> 
  group_by(region) |> 
  summarise_at(vars("qt_kit01":"qt_cri_other"), mean, na.rm = TRUE) |> 
  mutate_at(vars("qt_kit01":"qt_cri_other"), 
            list(~. * projectiondays)) |> 
  mutate_at(vars("qt_kit01":"qt_cri_other"), 
            list(~round(., digits = 0))) |> 
    pivot_longer(cols = c("qt_kit01":"qt_cri_other"), names_to="Tipo", values_to="Total") |>
...
kits_cris_list_trend_detail_socios <- rudah_all |>
  ...
  dplyr::filter(tipo_socio != "other") |> 
    select(region, today, organizacion, "qt_kit01":"qt_kit_other", "qt_cri01":"qt_cri_other") |>
  mutate_at(vars(starts_with("qt_kit")), ~ replace(., is.na(.), 0)) |>
  mutate_at(vars(starts_with("qt_kit")), as.numeric) |>
  mutate_at(vars(starts_with("qt_cri")), ~ replace(., is.na(.), 0)) |>
  mutate_at(vars(starts_with("qt_cri")), as.numeric) |>
